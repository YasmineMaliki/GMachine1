...
public class MachineService implements IDao<Machine> {
    ...
    public Boolean findReferenceExpectId(String reference,int id) {
    ...
    public List<Machine> findMarque(int id) {
        List<Machine> machines = new ArrayList<Machine>();
        ...
        try {
            ...
            System.out.println("FindMarque " + e.getMessage());
        }
        return machines;
    }
    ...
    public Map<String,Integer> findMachinesLesPlusAchete() {
    	...
    	MarqueService ms = new MarqueService();
        try {
            String req = "select m.libelle,count(*) from marque m, machine o where m.id=o.idMarque group by m.id order by count(*) desc limit 5";
            PreparedStatement pr =Connexion.getInstane().getConnection().prepareStatement(req);
            ResultSet rs = pr.executeQuery();
            while (rs.next()) {
